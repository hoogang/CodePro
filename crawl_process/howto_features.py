...
def featuring(instance, code_type):
  """ Feature engineering.
  Args:
    instance: {title, answer_text_code_blocks, question_post_text_code_blocks} in dict format,
  ...
  if code_type == "python":
    ...
    model = pickle.load(open("./code_classifier/code_classifier.model",'rb'))
  ...
  # keyword feature
  keyword_feature = keyword_featuring(" ".join([" ".join(text) for text in ans_text_tokenized] + [" ".join(text) for text in question_text_tokenized]),  howto_kw + debug_kw + other_kw)
  ...
  howto_kw_feature = sum(list(keyword_featuring( " ".join([" ".join(text) for text in ans_text_tokenized] + [" ".join(text) for text in question_text_tokenized]),  howto_kw, True, "howto").values()))
  feature['howto_kw_feature'] = howto_kw_feature

  debug_kw_feature = sum(list(keyword_featuring(" ".join([" ".join(text) for text in ans_text_tokenized] + [" ".join(text) for text in question_text_tokenized]), debug_kw, True, "debug").values()))
  feature['debug_kw_feature'] = debug_kw_feature

  other_kw_feature = sum(list(keyword_featuring( " ".join([" ".join(text) for text in ans_text_tokenized] + [" ".join(text) for text in question_text_tokenized]), other_kw, True, "other").values()))
...
title='keep table(rows) together with OpenXML SDK 2.5'

str1='''
<body><p>I'd like to generate multiple tables with 2 rows each inside of a word document. But I want to keep these two rows together (if possible).</p>
<ol>
<li><code>new KeepNext()</code>on the first row doesn't work </li>
<li><code>new KeepNext()</code>on the last paragraph of the first row doesn't work</li>
<li><code>new CantSplit()</code> on the table doesn't work</li>
</ol>
<p>In all cases the second row (if too large) lands on the second page. A <code>new CantSplit()</code> on the last cell ( the one with large content) avoids a break of the cell. But none of the options avoids a split of the table (rowwise).</p>
</body>
  '''

str2='''
<body><p>I'd like to generate multiple tables with 2 rows each inside of a word document. But I want to keep these two rows together (if possible).</p>
<pre><code>i did it like this when I had to apply this to all tables:
...
<p>getting all tables</p>

<pre><code>var t = package.MainDocumentPart.Document.Body.Descendants&lt;Table&gt;().ToList()
</code></pre>
<body>
'''

properties = {'title':title, 'answer_text_code_blocks':[str2,str2],'question_text_code_blocks':[str1,str1]}


if __name__ == '__main__':
  featuring(properties, 'python')
