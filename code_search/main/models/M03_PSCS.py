...
class SiameseCSNN(object):
    ...
    def weight_embed(self,seq_embeds,scope):
        #  (?, 20,300) - (?,300)
        mean_embeds= tf.reduce_mean(seq_embeds, axis=1, keep_dims=False)
        # （？，20,300） = 300
        dim = seq_embeds.get_shape().as_list()[2]
        #  (?,300,300)
        w= tf.get_variable('w'+scope, shape=[dim,dim],initializer=tf.truncated_normal_initializer(stddev=0.1))
        #  (?,20,300) * (300,300) = (?,20,300) * (?,300) = (?,20)
        att= tf.nn.softmax(tf.einsum('abc,ac->ab',tf.einsum('abc,cd->abd',seq_embeds,w),mean_embeds))
        #  (?,20,300) * (?,20)
        att_out = tf.einsum('abc,ab->ac',seq_embeds,att)
    ...
    def build(self, struc2vecs):
        ...
        with tf.variable_scope('attweight') as scope0:
            ...
            q_tanh = tf.nn.tanh(self.weight_embed(q_embed, 'query'))
