...
def main(corpus_folder,data_folder,lang_type):
    ...
    corpus_qid_allterms = pickle.load(open(corpus_folder+'%s_saihnn_corpus_qid_allterms.pickle' % lang_type, 'rb'))
    corpus_qids = set([i[0][0] for i in corpus_qid_allterms])

    # 读取codedb数据
    # [(id, index),[[c]] 代码，[q] 查询, 块长度，标签]
    codedb_blocks = pickle.load(open(data_folder+'%s_codedb_qid2index_blocks_unlabeled.pickle' % lang_type, 'rb'))
    ...
    print('%s数据最开始爬取的原始长度是%d' % (lang_type, attr_length))
    # 开始循环查找，数据长度
    lang_lencut = math.floor(attr_length / (2**4)) * (2**4)
    print('%s数据用小波变换的截断长度是%d' % (lang_type, lang_lencut))
    '''
    多选      sql      python 
        开始  687254    884909 
        截断  687248    884896
    ...
    print('%s数据用小波变换需截取的长度是%d'%(lang_type, remove_lencut))

    # qid 差集  corpus是否包含codedb
    # 经判定corpus语料是包含coded评测语料的
    # (staqc多候选包含hnn)
    judge_cover = [codedb_qids.issubset(corpus_qids)]
    # 打印True 覆盖
    print(judge_cover)

    # 两者不同的qid部分
    differ_qids = list(set(corpus_qids).difference(set(codedb_qids)))

    print('%s在corpus和codedb差值长度是%d' % (lang_type, len(differ_qids)))
    # 去除的qid 部分
    remove_qids = random.sample(differ_qids, remove_lencut)
    ...
    print('%s数据用小波变换保存的长度为%d'%(lang_type,len(attr_data)))
    ...
    # 保存前项索引值
    sort_attr.to_csv(data_folder+'%s_saihnn_corpus_qids_scored.csv' %lang_type, sep='|', encoding='utf-8')
