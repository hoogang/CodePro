...
def main(corpus_folder,data_folder,lang_type):
    ...
    # (staqc多候选包含hnn)
    judge_cover = [codeqa_qids.issubset(corpus_qids)]
    # 打印True 覆盖
    print(judge_cover)

    # 两者不同的qid部分
    differ_qids = list(set(corpus_qids).difference(set(codeqa_qids)))

    print('%s在corpus和codeqa差值长度是%d' % (lang_type, len(differ_qids)))
    # 去除的qid 部分
    ...
    # 保存前项索引值
    sort_attr.to_csv(data_folder+ '%s_codemf_corpus_qids_scored.csv' %lang_type, sep='|', encoding='utf-8')
