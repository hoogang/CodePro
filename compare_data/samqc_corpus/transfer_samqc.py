...
def pickle_tocsv(lang_type,save_path):
    ...
    print('%s单候选的qid-index长度为%s' % (lang_type, len(single_code)))
    ##############################################打印测试######################################################
    #[16384000, 26258091]
    print(s3, '\n')
    ...
    for id in list(single_query.keys()):
        ...
        # 随机再选2个候选代码 !!!
        cor = [i for i in list(single_code.items()) if i[0]!=id]
        cor = random.sample(cor,2)

        #  第一个候随机代码
        single_query_list.append(filter_inva_char(single_query[id]))
        single_qid_list.append(id)
        single_cid_list.append(1)
        code1 = filter_inva_char(cor[0][1])
        single_code_list.append(code1)

        # 第二个随机代码
        single_query_list.append(filter_inva_char(single_query[id]))
        single_qid_list.append(id)
        single_cid_list.append(2)
        code2 = filter_inva_char(cor[1][1])
        single_code_list.append(code2)

    print('单候选代码转换完毕！')

    #  查询索引
    qid_list  = mutiple_qid_list +single_qid_list
    #  代码索引
    cid_list  = mutiple_cid_list +single_cid_list
    #  查询字符
    query_list= mutiple_query_list+single_query_list
    #  代码字符
    code_list = mutiple_code_list+single_code_list
    ...
    mutiple_data.to_csv(save_path+'mutiple_samqc_%s.csv'%lang_type,encoding='utf-8',sep='|')
