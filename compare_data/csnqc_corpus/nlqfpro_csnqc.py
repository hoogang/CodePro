...
def load_vocab_model(lang):
    with open('../../nlqf_tool/resource/word_vocab.json','r') as f:
        word_vocab = json.load(f)
    model_file = torch.load('../../nlqf_tool/resource/java_vae.model')
...
def jsonl_tocsv(data_folder,lang_type,save_path):
    ...
    print('filter before:',len(raw_docs))        #496688

    filter_docs, idx = ruler.filter(raw_docs)
    print('rule filter:', len(filter_docs))      #350147
    # idx是保留有用注释的索引
    ...
    print('filter after:',len(finall_docs))   #236335
    ...
    print('原始所有数据的大小', single_data.shape)  #236335
    ...
    # 保存csv
    single_data.to_csv(save_path + 'nlqfpro_csnqc_%s.csv'%lang_type, encoding='utf-8', sep='|')
    print('\n%s数据类型转换完毕！！'%lang_type)
